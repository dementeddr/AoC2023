...
def main(data):
    ...
    for line in data:
        ...
        halves = line.split()
        nums = ','.join([halves[1] for i in range(5)])
        nums = list(map(int,num_pat.findall(nums)))
        springs = '?'.join([halves[0] for i in range(5)])
...
def does_fit(section, nums, indent):
    ...
    key = (section, tuple(nums))
    value = memo.get(key, None)

    if value is not None:
        value = memo[key]
        log(3, f"{indent}Found {key} = {value} in memo.")
        return value
    ...
    memo[key] = total_fits
    log(3, f"{indent}memoizing {key} = {total_fits}")
